...
def fullgrad(
    ...
    **kwargs,
    ...
    bias_terms: List[Tensor] = []
    hooks = []

    def _register_bias_hook(module: torch.nn.Module) -> None:
        ...
        def bwd_hook(_mod, _grad_in, grad_out):
            ...
            b = module.bias
            while b.dim() < g.dim():
                b = b.view(*([1] * (g.dim() - 1)), -1)
            contrib = (g * b).sum(dim=-1)
            if contrib.dim() >= 2:
                if contrib.shape[1] == L:
                    if contrib.dim() > 2:
                        contrib = contrib.mean(dim=tuple(range(2, contrib.dim())))
                    bias_terms.append(contrib)
                else:
                    val = contrib.view(contrib.shape[0], -1).mean(dim=1, keepdim=True)
                    bias_terms.append(val.repeat(1, L))
            else:
                bias_terms.append(contrib.view(contrib.shape[0], 1).repeat(1, L))

        hooks.append(module.register_full_backward_hook(bwd_hook))

    for mod in model.modules() if isinstance(model, torch.nn.Module) else []:
        if isinstance(mod, (torch.nn.Linear, torch.nn.LayerNorm, torch.nn.Conv1d, torch.nn.Conv2d, torch.nn.Conv3d)):
            _register_bias_hook(mod)
    ...
    if bias_terms:
        stack = torch.stack(bias_terms, dim=0)
        bias_attr = stack.sum(dim=0)
        feature_attr = feature_attr + bias_attr
